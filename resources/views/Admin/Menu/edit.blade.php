...
<div class="content">
    <div class="box card">
        <form role="form" enctype="multipart/form-data" method="post"
              action="{{ route('menus.update',['menu' => $item->id ]) }}">
            @csrf
            @method('PUT')
                                        ...
                                        <div class="col-lg-10">
                                            <div class="input-group">
                                                <input type="text" name="path" class="form-control"
                                                       value="{{ $item->path ?? __('fields.Alias.Value') }}"
                                            ...
                                            <div class="col-lg-10">
                                                <div class="input-group">
                                                    <select name="parent" class="form-control select2"
                                                            style="width: 100%;">
                                                        <option
                                                            {{ $item->parent === Null ?'selected':'' }} value="0">{{__('ParentNull')}}</option>
                                                        @foreach($menus as $menu)
                                                            <option
                                                                {{ $item->id === $menu->id? 'disabled' :'' }}
                                                                {{ $item->parent === $menu->id? 'selected' :'' }}
                                                                value="{{ $menu->id ?? "" }}">
                                        ...
                                        <div class="col-lg-10">
                                            <div class="input-group">
                                                <select name="type" class="form-control select2" style="width: 100%;">
                                                    <option {{ $item->type === 'admin'? 'selected' :'' }} value="admin">
                                                        admin
                                                    </option>
                                                    <option {{ $item->type === 'front'? 'selected' :'' }} value="front">
                                                        front
                                                    </option>
                                                </select>
                                            </div>
