...
class ModuleMake extends Command
{
    ...
    private function createApiController()
    {
        ...
        if ($this->alreadyExists($path)) {
            ...
            $stub = str_replace(
                ...
                [
                    $today,
                    "App\\Modules\\" . str_replace('/', '\\', trim($this->argument('name'))) . "\\Controllers\\Api",
                    $this->laravel->getNamespace(),
                    $controller . 'Controller',
                    "App\\Modules\\" . str_replace('/', '\\', trim($this->argument('name'))) . "\\Models\\{$modelName}",
                    $modelName,
    ...
    private function createMigration()
    {
        ...
        $path = str_replace('\\', '/', trim($this->argument('name')));

        try {
            $this->call('make:migration', [
                'name' => "create_{$table}_table",
                '--create' => $table,
                '--path' => "app/Modules/" . $path . "/Migrations"
            ]);
        } catch (\Exception $e) {
    ...
    private function createService()
    {
        ...
        if ($this->alreadyExists($path)) {
            ...
            $stub = str_replace(
                ...
                [
                    $today,
                    "App\\Modules\\" . str_replace('/', '\\', trim($this->argument('name'))) . "\\Services",
                    "App\\Modules\\" . str_replace('/', '\\', trim($this->argument('name'))) . "\\Requests\\{$modelName}Request",
                    "{$modelName}Request",
                    "{$modelName}Service",
                ],
                $stub
    ...
    {
        $model = Str::singular(Str::studly(class_basename($this->argument('name'))));
        return $this->laravel['path'] . '/Modules/' . str_replace('\\', '/', $name) . "/Services/" . "{$model}Service.php";
    ...
    private function createRequest()
    {
        ...
        $path = $this->getRequestPath($this->argument('name'));
        ...
        if ($this->alreadyExists($path)) {
            ...
            $stub = str_replace(
                ...
                [
                    $today,
                    "App\\Modules\\" . str_replace('/', '\\', trim($this->argument('name'))) . "\\Requests",
                    "{$modelName}Request",
                    $AccessName
                ],
                $stub
    ...
    private function getRequestPath($name)
    {
        $model = Str::singular(Str::studly(class_basename($this->argument('name'))));
        return $this->laravel['path'] . '/Modules/' . str_replace('\\', '/', $name) . "/Requests/" . "{$model}Request.php";
    ...
    private function createPolicy()
    {
        ...
        $path = $this->getPolcyPath($this->argument('name'));
        ...
        if ($this->alreadyExists($path)) {
            ...
            $stub = str_replace(
                ...
                [
                    $today,
                    "App\\Modules\\" . str_replace('/', '\\', trim($this->argument('name'))) . "\\Policies",
                    "App\\Modules\\" . str_replace('/', '\\', trim($this->argument('name'))) . "\\Models\\{$modelName}",
                    "{$modelName}Policy",
                    $AccessName,
                    $modelName,
    ...
    private function getPolcyPath($name)
    {
        $model = Str::singular(Str::studly(class_basename($this->argument('name'))));
        return $this->laravel['path'] . '/Modules/' . str_replace('\\', '/', $name) . "/Policies/" . "{$model}Policy.php";
